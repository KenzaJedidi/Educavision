...
class CandidatureController extends AbstractController
{
    ...
    public function changeStatus(
        ...
        $oldStatus = $candidature->getStatut();
        $candidature->setStatut($status);
        $em->flush();

        // Send email notification to the teacher if status changed to Accepté or Refusé
        if ($status !== $oldStatus && in_array($status, ['Accepté', 'Refusé'], true)) {
            $candidatureEmail = $candidature->getEmail();
            $teacher = $utilisateurRepository->findOneBy(['email' => $candidatureEmail]);

            if ($teacher) {
                try {
                    if ($status === 'Accepté') {
                        // Promote teacher role if not already
                        if ($teacher->getRole() !== 'professeur' && $teacher->getRole() !== 'admin') {
                            $teacher->setRole('professeur');
                            $teacher->setDateModification(new \DateTime());
                            $em->flush();
                        }
                        $emailService->sendTeacherCvAcceptedEmail($teacher, $candidature);
                    } else {
                        $emailService->sendTeacherCvRejectedEmail($teacher, $candidature);
                    }
                } catch (\Exception $e) {
                    // Email failed silently — status change is still saved
                }
            }
        }
